...
{
	inline std::string name = "External VRZ";
	inline std::string version = "1";
	inline std::string author = "Vrosik";
...
namespace ESPConfig
{
	...
	// Item ESP colors (for inventory)
	inline ImColor BombColor = ImColor(255, 165, 0, 180);
	inline ImColor WeaponColor = ImColor(255, 255, 255, 255);

	// Dropped items ESP colors (for items on ground) - currently disabled
	// inline bool ShowDroppedItems = true;
	// inline bool ShowDroppedBomb = true;
	// inline bool ShowDroppedWeapons = true;
	// inline bool ShowDroppedGrenades = true;

	// inline ImColor DroppedBombColor = ImColor(255, 165, 0, 255);
	// inline ImColor DroppedWeaponColor = ImColor(150, 150, 255, 255);
	// inline ImColor DroppedGrenadeColor = ImColor(100, 200, 100, 255);
}
...
namespace AimConfig
{
	...
	inline int AimKey = VK_LBUTTON;  // Left mouse button
	inline int AimKeyMode = 0;      // 0: Hold, 1: Toggle
	inline bool AimToggle = false;
	inline float AimFov = 5.0f;     // FOV in degrees
	inline float AimSmooth = 2.0f;  // Smoothing factor
	inline int AimBone = 8;         // Head bone by default (BONEINDEX::head)
	inline bool VisibleCheck = true;
	inline bool TeamCheck = true;
	inline float MaxAimDistance = 500.0f;
	inline bool DrawFov = false;
	inline ImColor AimFovColor = ImColor(255, 0, 0, 100);
}
