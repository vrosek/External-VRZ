...
namespace ESP
{
	...
	void RenderPlayerESP(const CEntity& LocalEntity, const CEntity& Entity, ImVec4 Rect, int LocalPlayerControllerIndex, int Index)
	{
		...
		if (ESPConfig::ShowWeaponESP) {
			// Check if player has C4 in inventory
			auto weapons = Entity.Pawn.GetWeaponInventory(gGame.GetEntityListEntry());
			bool hasC4 = false;
			...
			// Show bomb icon on the left if player has C4 (slightly dimmed)
			if (hasC4) {
				std::string bombIcon = GunIcon("c4");
				if (!bombIcon.empty()) {
					ImVec2 bombPos = { Rect.x + Rect.z / 2 - 16, Rect.y + Rect.w + 2 };

					// Тень для бомбы
					const ImVec2 shadowOffsets[4] = { { -1, -1 }, { -1, 1 }, { 1, 1 }, { 1, -1 } };
					for (const auto& off : shadowOffsets) {
						ImVec2 pos = { bombPos.x + off.x, bombPos.y + off.y };
						ImGui::GetBackgroundDrawList()->AddText(ioFonts, 12.0f, pos, ImColor(0, 0, 0, 150), bombIcon.c_str());
					}

					// Основная иконка бомбы
					ImColor bombColor = ImColor(255, 165, 0, 180);
					ImGui::GetBackgroundDrawList()->AddText(ioFonts, 12.0f, bombPos, bombColor, bombIcon.c_str());
				}
			}

			// Show current weapon in center
			std::string weaponIcon = GunIcon(Entity.Pawn.WeaponName);
			if (!weaponIcon.empty()) {
				ImVec2 weaponPos = { Rect.x + Rect.z / 2, Rect.y + Rect.w + 2 };

				// Тень для лучшей видимости
				const ImVec2 shadowOffsets[4] = { { -1, -1 }, { -1, 1 }, { 1, 1 }, { 1, -1 } };
				for (const auto& off : shadowOffsets) {
					ImVec2 pos = { weaponPos.x + off.x, weaponPos.y + off.y };
					ImGui::GetBackgroundDrawList()->AddText(ioFonts, 12.0f, pos, ImColor(0, 0, 0, 255), weaponIcon.c_str());
				}

				// Основная иконка
				ImColor weaponColor = ImColor(255, 255, 255, 255);
				ImGui::GetBackgroundDrawList()->AddText(ioFonts, 12.0f, weaponPos, weaponColor, weaponIcon.c_str());
			}
		}
   ...
   if (ESPConfig::ShowIsScoped) {
    ...
    // Dropped items ESP removed - functionality disabled
    ...
    void RenderPreview(ImVec2 windowSize)
    {
        ...
        // Weapon icon display in preview (inventory)
        if (ESPConfig::ShowWeaponESP) {
            // C4 bomb on the left
            ImColor bombPreviewColor = ImColor(255, 165, 0, 180);
            drawList->AddText(font1, 12.0f, { centerPos.x + 24, centerPos.y + rectSize.y + 8 }, bombPreviewColor, "o");
            // Current weapon in center
            ImColor weaponPreviewColor = ImColor(255, 255, 255, 255);
            drawList->AddText(font1, 12.0f, { centerPos.x + 36, centerPos.y + rectSize.y + 8 }, weaponPreviewColor, "W");
        }

        // Dropped items preview removed
        if (ESPConfig::ShowIsScoped) {
